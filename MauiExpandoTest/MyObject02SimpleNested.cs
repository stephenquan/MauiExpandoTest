...
public partial class MyObject02SimpleNested : DynamicObject, INotifyPropertyChanged
{
	ExpandoObject InternalValue { get; } = new();
	public dynamic Dynamic => (dynamic)InternalValue;

	public MyObject02SimpleNested()
	{
	...
	void SetJsonObject(ExpandoObject obj, JsonElement.ObjectEnumerator objectEnumerator)
	{
		...
		foreach (var property in objectEnumerator)
		{
			switch (property.Value.ValueKind)
			{
				...
				case JsonValueKind.Object:
					...
					{
						//PropertyChanged?.Invoke(this, new PropertyChangedEventArgs($"Item[{property.Name}]"));
						PropertyChanged?.Invoke(this, new PropertyChangedEventArgs(nameof(Json)));
					};
					SetJsonObject(subObject, property.Value.EnumerateObject());
